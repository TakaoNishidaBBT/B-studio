...
//				var w = jQuery(el).parent().width() + scrollbar;
	...
	resize: function() {
		...
		this._changing(this.id + '-lhs', this.id + '-rhs');
		this.editor[this.id + '-lhs'].refresh();
		this.editor[this.id + '-rhs'].refresh();
	},
	diff: function() {
...
//			if (!top_offset) return;//try again
	...
	_draw_diff: function(editor_name1, editor_name2, changes) {
		...
		ctx_lhs.beginPath();
		ctx_lhs.fillStyle = this.settings.bgcolor;
		ctx_lhs.strokeStyle = '#ccc';
		ctx_lhs.fillRect(0, 0, 8, ex.visible_page_height);
		ctx_lhs.strokeRect(0, 0, 8, ex.visible_page_height);

		ctx_rhs.beginPath();
		ctx_rhs.fillStyle = this.settings.bgcolor;
		ctx_rhs.strokeStyle = '#ccc';
		ctx_rhs.fillRect(0, 0, 8, ex.visible_page_height);
		ctx_rhs.strokeRect(0, 0, 8, ex.visible_page_height);
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			ctx_lhs.strokeStyle = '#444';
			ctx_lhs.lineWidth = 0.5;
			ctx_lhs.fillRect(1.0, lhs_y_start, 6.0, Math.max(lhs_y_end - lhs_y_start, 5));
			ctx_lhs.strokeRect(1.0, lhs_y_start, 6.0, Math.max(lhs_y_end - lhs_y_start, 5));

			ctx_rhs.beginPath();
			ctx_rhs.fillStyle = this.settings.fgcolor[change['op']];
			ctx_rhs.strokeStyle = '#444';
			ctx_rhs.lineWidth = 0.5;
			ctx_rhs.fillRect(1.0, rhs_y_start, 6.0, Math.max(rhs_y_end - rhs_y_start, 5));
			ctx_rhs.strokeRect(1.0, rhs_y_start, 6.0, Math.max(rhs_y_end - rhs_y_start, 5));
