...
*/
	bframe.addEventListner(window, 'load' , bframeCompareTreeInit);
	...
	bframe.compare_tree_container = function() {
		...
		this.selectRelativeNode = function(target, obj) {
			for(var i=0 ; i<trees.length ; i++) {
				if(trees[i].getTarget() == target) continue;

				var p = bframe.serachNodeByNameAndValue(trees[i].getTarget(), 'path', obj.value);
				if(!p) {
					var relId = trees[i].getIndex() + obj.id.substr(1, 1) + trees[i].getIndex() + obj.id.substr(3);
					p = bframe.searchNodeById(trees[i].getTarget(), relId);
					if(!p) {
				...
				if(pain) {
					if(current_node.id() && node_info.node_id == current_node.id().substr(1)) {
						...
						if(pain_disp_change && pain_disp_change_select.options[pain_disp_change_select.selectedIndex].value == 'detail') {
							...
							div.appendChild(pul);
							for(var i=0 ; i < node_info.children.length ; i++) {
								createNodeObject(pul, node_info.children[i], 'pain');
							}
						}
					}
					var ul = document.createElement('ul');
...
*/
		...
		function openNode(node_id) {
			...
			param = 'terminal_id='+terminal_id+'&node_id='+node_id.substr(2);
		...
		function createNodeObject(parent, config, place) {
			...
			if(config.node_type != 'root' && config.node_type != 'trash' && config.node_status) {
				var icon = config.node_type + '-' + config.node_status;
			}
			else {
...
//console.log(icon);
		...
		function selectNode(event) {
			...
			var p = bframe.serachNodeByName(node, 'path');
console.log(p);
			tc.selectRelativeNode(target, p);
