...
*/
	class widget_compare_tree extends B_AdminModule {
		...
		function view() {
			...
			// HTML ヘッダー出力
			$this->html_header->appendProperty('css', '<link href="css/widget_compare_tree.css" type="text/css" rel="stylesheet" media="all" />');
		...
		function compare(&$left, &$right) {
			...
			$sql = "select a.*, a.contents_id updated_contents, c.cnt
					...
					,(select max(concat(version_id, revision_id)) version, node_id, count(*) cnt from " . B_DB_PREFIX . B_WIDGET_NODE_TABLE . "
						where node_id in(
							select node_id
							from " . B_DB_PREFIX . B_WIDGET_NODE_TABLE . " a
