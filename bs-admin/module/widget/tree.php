...
*/
	class widget_tree extends B_AdminModule {
		...
		function _callback_copy(&$node) {
			...
			$ret = $this->copyWidget($node->contents_id
									, $node->version
									, $node->revision
									, $new_contents_id);
		...
		function copyWidget($contents_id, $version_id, $revision_id, &$new_contents_id) {
			$table_widget = new B_Table($this->db, B_WIDGET_TABLE);

			$sql = "select * from " . B_DB_PREFIX . B_WORKING_WIDGET_VIEW . " where contents_id='" . $contents_id . "'";
			...
			$ret = $table_widget->selectInsert($row);
			$new_contents_id = $table_widget->selectMaxValue('contents_id');
