...
*/
	class article2_form extends B_AdminModule 		public $settings;
		public $tab_control;
		public $editor;
		public $result;
		...
		function copy() {
			if($this->post['source_id'] && is_numeric($this->post['source_id'])) {
				$sql = "select * from " . B_DB_PREFIX . "v_admin_article2 where article_id = %id%";
		...
		function getCategory() {
			$sql = "select * from " . B_DB_PREFIX . "v_category2 order by disp_seq";
			$rs = $this->db->query($sql);
