	...
	class article3_form extends B_AdminModule {
		...
		function select() {
			...
			default:
				...
				$sql = "select * from " . B_DB_PREFIX . "v_admin_article3 where article_id = '$article_id'";
		...
		function getCategory() {
			$sql = "select * from " . B_DB_PREFIX . "v_category3 where parent_node = 'root' order by disp_seq";
			$rs = $this->db->query($sql);
		...
		function checkSlug($article_id, $slug) {
			...
			$sql = "select count(*) cnt from " . B_DB_PREFIX . "article3 where slug = binary '$slug' and article_id <> '$article_id'";
			$rs = $this->db->query($sql);
			$row = $this->db->fetch_assoc($rs);
		...
		function deleteTag($article_id) {
			$sql = "delete from " . B_DB_PREFIX . "article_tag3 where article_id='$article_id'";
		...
		function insertTag($article_id, $tag_id) {
			...
			$table_article_tag = new B_Table($this->db, 'article_tag3');
