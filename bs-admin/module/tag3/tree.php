...
*/
	class tag3_tree extends B_AdminModule {
		function __construct() {
			parent::__construct(__FILE__);

			if(array_key_exists('tag_id', $this->request)) {
				$selected = explode(',', $this->request['tag_id']);
				foreach($selected as $value) {
					if($selected_nodes) $selected_nodes.= ',';
					$node_id = 't' . $value;
					$selected_nodes.= $node_id;
				}
				$this->session['selected_nodes'] = $selected_nodes;
			}
		...
		function openCurrentNode($node_id) {
			...
			$sql_org = str_replace('%VIEW%', B_DB_PREFIX . B_TAG3_TABLE, $sql_org);
		...
		function view() {
			...
			// HTML ヘッダー出力
			$this->html_header->appendProperty('css', '<link rel="stylesheet" href="css/tag.css">');
		...
		function createNode() {
			...
			$ret = $this->checkNodeType($node->node_type);
			if($ret) {
				// start transaction
				$this->db->begin();
		...
		function saveName() {
			if($this->request['node_id'] && $this->request['node_id'] != 'null') {
				...
				else if(strpos($this->request['node_name'], ',')) {
					$this->status = false;
					$this->message = __('Cannot use ,(Comma) for Tag Name');
				}
		...
		function response($node_id, $category) {
			...
			if($this->session['selected_nodes']) {
				$response['current_nodes'] = $this->session['selected_nodes'];
			}
			if($this->message) {
		...
		function checkNodeType($parent_node_type) {
			switch($parent_node_type) {
			case 'root':
				return true;

			case 'tag':
				$this->message = __('Sorry, Tag can be created at root level.');
				return false;
			}
		}
	}
